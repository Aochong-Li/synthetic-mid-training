#!/usr/bin/env python3
"""
Parallel downloader for PKG2 dataset files.
Downloads files in parallel using 8 workers with progress tracking and error handling.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pkg2_download.log'),
...
def load_urls(links_file: str) -> List[Tuple[str, str]]:
    """
...
def main():
    ...
    logger.info(f"⏱️  Time elapsed: {elapsed_time:.2f} seconds ({elapsed_time/60:.2f} minutes)")
    logger.info("=" * 80)
    ...
    logger.info(f"\n💾 Total downloaded: {total_size_gb:.2f} GB")
